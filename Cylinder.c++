...
Cylinder::Cylinder(ShaderIF* sIF, cryph::AffPoint botIn, double radiusIn, double heightIn, float colorIn[]) : shaderIF(sIF)
{
  //might add shaderIF here
  radius = radiusIn;
  height = heightIn;
  m_bot = botIn; //center of bottom of cylinder
  m_top = cryph::AffPoint(m_bot.x, m_bot.y, m_bot.z+height); //center of top of cylinder
  xyz[0] = m_bot.x - radius; xyz[1] = m_bot.x - radius; //x limits
  xyz[2] = m_bot.y - radius; xyz[3] = m_bot.y - radius; //y limits
...
void Cylinder::defineCylinder()
{
  ...
  for (int i=0 ; i<=N_POINTS_AROUND_SLICE ; i++)
	{
    cryph::AffPoint bot = m_bot + radius * (cos(theta)*U + sin(theta)*V);
    cryph::AffPoint top = m_top + radius * (cos(theta)*U + sin(theta)*V);
		theta += dTheta;
    int j = i*2;
